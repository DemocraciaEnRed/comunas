...
CommentsView.prototype.load = function(comments) {
  ...
  if (bestInFavor) {
    this.infavorCard = new CommentCard(bestInFavor);
    this.infavorCard.appendTo(this.infavorContainer);
  } else {
    this.infavorContainer.html(t('comments.best.nonexistent.infavor'));
  }

  var bestAgainst = this.getBestAgainst();
  if (bestAgainst) {
    this.againstCard = new CommentCard(bestAgainst);
    this.againstCard.appendTo(this.againstContainer);
  } else {
    this.againstContainer.html(t('comments.best.nonexistent.against'));
  }
