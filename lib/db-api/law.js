...
exports.all = function all(fn) {
  ...
  Law
  .find({ deletedAt: null })
  .select('id lawId mediaTitle tag participants votes createdAt updatedAt closingAt publishedAt deletedAt status open closed links author authorUrl keywords')
  .populate('tag', 'id hash name color image')
  .populate('keywords', 'id name')
...
exports.rss = function all(fn) {
  ...
  Law
  .find({ deletedAt: null })
  .select('id lawId mediaTitle publishedAt textMain textSecondary')
  .exec(function (err, laws) {
    if (err) {
      log('Found error %j', err);
      return fn(err);
