...
app.get('/law/all', function (req, res) {
  ...
  api.law.all(function(err, lawDocs) {
    ...
    var keys = [
      ...
      'tag participants voted createdAt updatedAt closingAt keywords',
...
app.get('/law/:id', function (req, res) {
  ...
  api.law.get(req.params.id, function (err, lawDoc) {
    ...
    var keys = [
      'id lawId title mediaTitle textMain textSecondary clauses source status open closed public draft deleted',
      'satus state tag participants closeMessage keywords',
      'upvotes downvotes abstentions createdAt updatedAt closingAt',
      'publishedAt deletedAt votable clauseTruncationText links author authorUrl creator'
    ].join(' ');
  ...
  api.law.get(req.params.id, function (err) {
    ...
    api.comment.getFor({context: 'summary', reference: re}, paging, function (err, comments) {
      ...
      var keys = [
        ...
        'author.id author.fullName author.avatar author.official author.organization',
...
app.post('/law/:id/comment', restrict, function (req, res) {
  ...
  comment.tendency = req.body.tendency;
...
app.post('/law/create', restrict, /*staff,*/ function (req, res, next) {
  ...
  var law = req.body;
  law.creator = req.user.id;

  if (!req.user.staff) {
    var closingAt = new Date();
    closingAt.setDate(closingAt.getDate() + config.lawOpenDays);
    closingAt = new Date(closingAt.getFullYear(), closingAt.getMonth(), closingAt.getDate() + 1);
    law.closingAt = law.closingAt || closingAt;
  }

  if (law.keywords) {
    api.keyword.findOrCreateByName(law.keywords.split(','), function(err, keywords){
      if (err) return _handleError(err, req, res);
      law.keywords = keywords;
      createLaw(law, req, res);
    });
  } else {
    createLaw(law, req, res);
  }
...
function createLaw(law, req, res) {
  api.law.create(law, function (err, lawDoc) {
    ...
    var keys = [
      ...
      'author authorUrl textMain textSecondary keywords'
    ].join(' ');
...
app.post('/law/:id', restrict, staff, function (req, res) {
  ...
  // Prevents law from being unpublished when updated #32
  if (law.publishedAt === null) {
    delete law.publishedAt;
  }
  ...
  if (law.keywords) {
    api.keyword.findOrCreateByName(law.keywords.split(','), function(err, keywords){
      if (err) return _handleError(err, req, res);
      law.keywords = keywords;
      updateLaw(req.params.id, law, req, res);
    });
  } else {
    law.keywords = [];
    updateLaw(req.params.id, law, req, res);
...
function updateLaw(id, law, req, res) {
  api.law.update(id, law, function (err, lawDoc) {
    if (err) return _handleError(err, req, res);

    log('Serving law %s', lawDoc.id);

    var keys = [
      'id lawId title mediaTitle clauses source state',
      'status open closed public draft deleted tag participants',
      'upvotes downvotes abstentions createdAt updatedAt closingAt',
      'publishedAt deletedAt votable clauseTruncationText links',
      'author authorUrl creator textMain textSecondary keywords'
    ].join(' ');
...
app.post('/law/:id/message', restrict, official, function (req, res) {
  ...
  api.law.get(req.params.id, function(err, lawDoc) {
    ...
    lawDoc.closeMessage = req.body.message;
    lawDoc.save(function (err, saved) {
      if (err) return _handleError(err, req, res);
