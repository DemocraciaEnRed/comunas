...
LawsFilter.prototype.reset = function() {
  this.initialize();
  this.set( { sort: 'closing-soon', status: 'open', 'hide-voted': false, search: '' } );
}
...
LawsFilter.prototype.reload = function() {
  ...
  if (search) items = items.filter(function (item) {
    if (~item.mediaTitle.toLowerCase().indexOf(search)) {
      return true;
    }

    if (item.keywords && item.keywords.length) {
      var keywords = item.keywords.map(function(k){ return k.name; }).join('');
      if (~keywords.indexOf(search)) return true;
    }
