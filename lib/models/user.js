...
var UserSchema = new Schema({
  ...
  , email:     { type: String, lowercase: true, trim: true, match: regexps.email, unique: true }
...
UserSchema.virtual('official').get(function() {
  return !!(config.officialUser && config.officialUser.toLowerCase() === this.email.toLowerCase());
});

UserSchema.virtual('organization').get(function() {
  var organizationUser = config.organizationUser || [];
  return !!~organizationUser.indexOf(this.email);
  // return !!(config.organizationUser && config.organizationUser.toLowerCase() === this.email.toLowerCase());
});
